...
test("Academy lessons directly reference forty-six valid, distinct 1600x1000 PNG images", async () => {
  ...
  assert.equal(hashes.size, 46, "all Academy lesson images must have unique SHA-256 hashes");
});
...
test("Academy lessons directly reference forty-six valid local M4A narrations", async () => {
  const lessons = getAcademyLessons("en");
  assert.equal(lessons.length, 46);
  const hashes = new Set<string>();
  ...
  assert.equal(hashes.size, 46, "all Academy narrations must have unique SHA-256 hashes");
});
