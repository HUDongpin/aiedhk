...
test("Academy lessons directly reference at least fifty-two valid, distinct 1600x1000 PNG images", async () => {
...
test("Academy lessons directly reference at least fifty-two valid local M4A narrations", async () => {
  const lessons = getAcademyLessons("en");
  assert.ok(lessons.length >= 52);
  const hashes = new Set<string>();
