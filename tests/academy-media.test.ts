...
test("Academy lessons directly reference thirty valid, distinct 1600x1000 PNG images", async () => {
  ...
  assert.equal(hashes.size, 30, "all Academy lesson images must have unique SHA-256 hashes");
});
...
test("Academy lessons directly reference thirty valid local M4A narrations", async () => {
  const lessons = getAcademyLessons("en");
  assert.equal(lessons.length, 30);
  const hashes = new Set<string>();
  ...
  assert.equal(hashes.size, 30, "all Academy narrations must have unique SHA-256 hashes");
});
