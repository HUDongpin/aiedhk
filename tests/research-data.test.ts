...
test("research news fallback contains ninety-one curated Research News items", () => {
  const papers = getResearchPapers("en");

  assert.equal(papers.length, 91);
  assert.equal(papers[0]?.slug, "news-source-grounded-ai-workflows-education");
...
test("the twenty-seven-date backlog contains exactly one research paper and one clearly labeled product-news item per day", () => {
  const papers = getResearchPapers("en");
...
test("backlog identifiers aied-038 through aied-091 are continuous and non-duplicated", () => {
  const ids = getResearchPapers("en")
    .map((paper) => Number.parseInt(paper.id.replace("aied-", ""), 10))
    .filter((id) => id >= 38 && id <= 91)
    .sort((a, b) => a - b);

  assert.deepEqual(ids, Array.from({ length: 54 }, (_, index) => index + 38));
});
...
test("backlog entries aied-038 through aied-091 have non-empty local M4A audio", () => {
  const backlog = getResearchPapers("en").filter((paper) => {
    const id = Number.parseInt(paper.id.replace("aied-", ""), 10);
    return id >= 38 && id <= 91;
  });

  assert.equal(backlog.length, 54);
  ...
  assert.equal(hashes.size, 54, "all backlog narrations must have unique SHA-256 hashes");
});
