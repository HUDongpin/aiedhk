...
- Assign exactly one image asset to each lesson and display it only once on the lesson detail page as the top hero. The same asset also serves as the listing-card and social-sharing image; do not add a separate cover or summary image.
