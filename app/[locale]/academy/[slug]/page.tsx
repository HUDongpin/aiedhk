...
import { academyLessons, getAcademyLessonBySlug, getRelatedAcademyLessons } from "@/lib/academy-data";
...
export default async function AcademyDetailPage({ params }: AcademyDetailPageProps) {
  ...
  const lesson = getAcademyLessonBySlug(slug, typedLocale);
  if (!lesson) notFound();
  ...
  return <div className="bg-hub-gradient">
    <JsonLd data={[academyLearningResourceJsonLd(lesson, typedLocale), breadcrumb]} />
    <article className="container-page py-14 lg:py-20">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[0.88fr_1.12fr] lg:items-start">
        <div className="relative aspect-[16/10] overflow-hidden rounded-4xl border border-slate-200 bg-aied-soft shadow-soft"><img src={lesson.image} alt={lesson.imageAlt} className="absolute inset-0 h-full w-full object-cover object-center" /></div>
        <div>
          ...
          <div className="mt-7">
            <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">{dictionary.academy.sources}</p>
            <div className="mt-3 flex flex-wrap gap-2">{lesson.sourceUrls.map((source) => <a key={source.url} href={source.url} target="_blank" rel="noreferrer" className="focus-ring inline-flex rounded-full bg-aied-ink px-5 py-2.5 text-sm font-black text-white transition hover:bg-aied-blue">{source.label} ↗</a>)}</div>
          </div>
        </div>
      </div>
      <div className="mt-12 grid gap-8 lg:grid-cols-[1fr_340px]">
        <div className="rounded-4xl border border-slate-200 bg-white p-7 shadow-card sm:p-9">
          ...
          <figure className="mt-6 overflow-hidden rounded-3xl border border-slate-200 bg-slate-50"><img src={lesson.summaryImage} alt={lesson.summaryImageAlt} className="block aspect-[16/10] h-auto w-full object-cover" /></figure>
          {lesson.summaryAudio ? <SummaryAudioPlayer src={lesson.summaryAudio} title={lesson.summaryAudioTitle} /> : null}
          <div className="research-summary mt-7 text-lg leading-8 text-aied-muted">{lesson.fullSummary.split("\n\n").map((paragraph, index) => <p key={index}>{paragraph}</p>)}</div>
        </div>
        <aside className="space-y-6">
          <div className="rounded-4xl border border-slate-200 bg-white p-7 shadow-card"><h2 className="text-xl font-black tracking-tight text-aied-ink">{dictionary.academy.coreIdeas}</h2><ol className="mt-5 space-y-5">{lesson.coreIdeas.map((idea, index) => <li key={idea} className="flex gap-3 text-base font-bold leading-7 text-aied-muted"><span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-aied-cyan text-xs text-slate-950">{index + 1}</span>{idea}</li>)}</ol></div>
          <div className="rounded-4xl border border-slate-200 bg-white p-7 shadow-card"><h2 className="text-xl font-black tracking-tight text-aied-ink">{dictionary.academy.educationConnection}</h2><p className="mt-4 text-base font-bold leading-7 text-aied-muted">{lesson.educationConnection}</p></div>
          <div className="rounded-4xl border border-slate-200 bg-white p-7 shadow-card"><h2 className="text-xl font-black tracking-tight text-aied-ink">{dictionary.academy.relatedConcepts}</h2><div className="mt-4 flex flex-wrap gap-2">{lesson.relatedConcepts.map((concept) => <span key={concept} className="rounded-full bg-aied-soft px-3 py-1.5 text-sm font-bold text-aied-blue">{concept}</span>)}</div></div>
        </aside>
      </div>
    </article>
